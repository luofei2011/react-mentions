...
var makeTriggerRegex = function(trigger) {
  ...
  function MentionsInput(_props) {
    ...
    _defineProperty(_assertThisInitialized(_this), "getInputProps", function(isTextarea) {
      ...
      var caretPosition = _this.state.caretPosition, _this$props5 = _this.props, suggestionsPortalHost = _this$props5.suggestionsPortalHost, allowSuggestionsAboveCursor = _this$props5.allowSuggestionsAboveCursor, placement = _this$props5.placement;
      if (caretPosition && _this.suggestionsRef) {
        ...
        if (suggestions) {
          ...
          if (suggestionsPortalHost) {
            ...
            var left = viewportRelative.left, _top = viewportRelative.top;
            left -= getComputedStyleLengthProp(suggestions, "margin-left"), _top -= getComputedStyleLengthProp(suggestions, "margin-top"), 
            left -= highlighter.scrollLeft, _top -= highlighter.scrollTop;
            ...
            allowSuggestionsAboveCursor && _top + suggestions.offsetHeight > viewportHeight && suggestions.offsetHeight < _top - caretHeight ? position.top = Math.max(0, _top - suggestions.offsetHeight - caretHeight) : position.top = _top;
          } else {
            var _left = caretPosition.left - highlighter.scrollLeft, _top2 = caretPosition.top - highlighter.scrollTop;
            _left + suggestions.offsetWidth > _this.containerRef.offsetWidth ? position.right = 0 : position.left = _left, 
            allowSuggestionsAboveCursor && viewportRelative.top - highlighter.scrollTop + suggestions.offsetHeight > viewportHeight && suggestions.offsetHeight < caretOffsetParentRect.top - caretHeight - highlighter.scrollTop ? position.top = _top2 - suggestions.offsetHeight - caretHeight : position.top = _top2;
          }
          var top = caretPosition.top - highlighter.scrollTop;
          position.top = "bottom" === placement ? top - suggestions.offsetHeight : top, isObjectEqual(position, _this.state.suggestionsPosition) || _this.setState({

  ...
  function MentionsInput(_props) {
    ...
    _defineProperty(_assertThisInitialized(_this), "updateSuggestionsPosition", function () {
      ...
      if (suggestionsPortalHost) {
        ...
        var _top = viewportRelative.top; // absolute/fixed positioned elements are positioned according to their entire box including margins; so we remove margins here:

        left -= getComputedStyleLengthProp(suggestions, 'margin-left');
        _top -= getComputedStyleLengthProp(suggestions, 'margin-top'); // take into account highlighter/textinput scrolling:

        left -= highlighter.scrollLeft;
        _top -= highlighter.scrollTop; // guard for mentions suggestions list clipped by right edge of window
        ...
        if (allowSuggestionsAboveCursor && _top + suggestions.offsetHeight > viewportHeight && suggestions.offsetHeight < _top - caretHeight) {
          position.top = Math.max(0, _top - suggestions.offsetHeight - caretHeight);
        } else {
          position.top = _top;
        }
      } else {
        ...
        var _top2 = caretPosition.top - highlighter.scrollTop; // guard for mentions suggestions list clipped by right edge of window
        ...
        if (allowSuggestionsAboveCursor && viewportRelative.top - highlighter.scrollTop + suggestions.offsetHeight > viewportHeight && suggestions.offsetHeight < caretOffsetParentRect.top - caretHeight - highlighter.scrollTop) {
          position.top = _top2 - suggestions.offsetHeight - caretHeight;
        } else {
          position.top = _top2;
        }
      }
